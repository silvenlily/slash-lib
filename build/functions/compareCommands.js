...
function compareOptions(current, goal) {
    ...
    for (let a = 0; a < goal.length; a++) {
        for (let b = 0; b < current.length; b++) {
            if (goal[a].name == current[b].name) {
                ...
                let optionGoal = goal[a];
                let optionCurrent = current[b];
                ...
                if (!optionCurrent.options || optionCurrent.options.length == 0) {
                    ...
                    if (optionGoal.options && optionGoal.options.length > 0)
                        return false;
                }
                else {
                    ...
                    if (!compareOptions(optionCurrent.options, optionGoal.options))
                        return false;
                }
                break;
            }
...
function compareCommands(current, goal) {
    if (!current) {
        return false;
    }
...
exports.default = compareCommands;
