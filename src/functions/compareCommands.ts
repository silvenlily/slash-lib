  ...
  for (let a = 0; a < goal.length; a++) {
    for (let b = 0; b < current.length; b++) {
      if (goal[a].name == current[b].name) {
        ...
        if (optionGoal.type != optionCurrent.type) return false;
        if (optionGoal.description != optionCurrent.description) return false;
        if (optionGoal.required ?? false != optionCurrent.required ?? false) return false;

        //  choices will always be: [{name:string, value:string|number}], this comparison should be safe
        if (JSON.stringify(optionGoal.choices ?? []) != JSON.stringify(optionCurrent.choices ?? []))
          return false;

        if (optionCurrent.options ?? [] == []) {
          // if current options is empty array or undifined then goal must be too
          if (optionGoal.options ?? [] != []) return false;
        } else {
...
function compareCommands(
  ...
  if (current.description != goal.description) return false;
  if (current.defaultPermission != (goal.defaultPermission ?? true)) return false;
  if (!compareOptions(current.options, goal.options)) return false;
